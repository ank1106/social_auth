...
class LoginView(View):
    ...
    def get(self, request):
        ...
        linkedin_callback_url = "{}://{}{}".format(
            request.META["wsgi.url_scheme"], request.META["HTTP_HOST"], reverse('linkedin'))
        return render(request, 'login.html', {"form": self.form_class(),
                                              "linkedin_key": os.environ["linkedin_key"],
...
class ProfileView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if not request.user.password or not profile.phone_number:
            return HttpResponseRedirect(reverse('set-password'))
...
class LinkedInLoginCallbackView(View):
    ...
    def get(self, request):
        ...
        r = requests.get('https://www.linkedin.com/uas/oauth2/accessToken',
                         params=payload, headers={"Content-Type": "application/json"})
        if r.status_code == 200:
            user_meta, email = get_linkedin_meta(r.json()["access_token"])
            user = create_update_user(email, "linkedin", user_meta, "{} {}".format(
                user_meta["localizedFirstName"], user_meta["localizedLastName"]))
            login(request, user)
            return HttpResponseRedirect('/accounts/profile/')
...
class TwitterLoginCallbackView(View):
    ...
    def get(self, request):
        ...
        data = {"oauth_verifier": request.GET.get(
            'oauth_verifier'),  "oauth_token": request.GET.get('oauth_token')}
        ...
        if access_token_data.status_code == 200:
            ...
            if user_data.status_code == 200:
                ...
                user = create_update_user(
                    user_data["email"], "twitter", user_data, user_data["name"])
                login(request, user)
                return HttpResponseRedirect('/accounts/profile/')
            return render(request, 'error.html')
        return render(request, 'error.html')
        ...
        if request.GET.get('code'):
            ...
            if r.status_code == 200:
                ...
                if data.get("access_token"):
                    emails = requests.get(
                        'https://api.github.com/user/emails', params={"access_token": data["access_token"][0]})
                    auth_result = requests.get(
                        'https://api.github.com/user', params={"access_token": data["access_token"][0]})
                    if auth_result.status_code == emails.status_code == 200:
                        email = [email["email"] for email in emails.json(
                        ) if email["primary"] and email["verified"]]
                        auth_result = auth_result.json()
                        if email:
                            user = create_update_user(
                                email[0], "github", auth_result, auth_result["login"])
                            login(request, user)
                            return HttpResponseRedirect('/accounts/profile/')
                    return render(request, 'error.html')
        return render(request, 'error.html')
